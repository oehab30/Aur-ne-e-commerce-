import React from 'react'
import { FaInstagram,FaFacebook } from "react-icons/fa";
import { FaXTwitter } from "react-icons/fa6";
import { Link } from 'react-router';
function Footer() {
  return (
    <div className='bg-black text-[#9CA3AF] p-20 flex gap-6'>


        <div>
            <h1 className='text-2xl text-white mb-6 '>Auréne</h1>
            <p className='w-70 mb-4'>Crafting luxury accessories that define elegance and sophistication.</p>
                 <div className='flex gap-3 text-xl' >
<Link to="">  <FaInstagram className='hover:text' /> </Link>
<Link to="">  <FaFacebook className='hover:text'/> </Link>
<Link to="">  <FaXTwitter className='hover:text'/> </Link>
                 </div>
        </div>


                         <div  className='flex-col duration-300'>
                            <h1 className='text-2xl text-white mb-6 '>Shop</h1>
                        <Link to="" className='hover:text-[var(--main)]'> Watches   </Link>
                        <Link to="" className='hover:text-[var(--main)]'> Bags   </Link>
                        <Link to="" className='hover:text-[var(--main)]'> Bracelets   </Link>
                        <Link to="" className='hover:text-[var(--main)]'> New Arrivals   </Link>
                     </div>
    </div>
  )
}

export default Footer